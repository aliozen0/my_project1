...
class ZED2LaneDetection:
    def __init__(self):
        self.bridge = CvBridge()
        self.image_sub = rospy.Subscriber("/zed2/zed_node/rgb/image_rect_color", Image, self.callback)
        self.line_pub = rospy.Subscriber("/huma/lineTracing",lineMessage,queue_size=10)
        self.frame = None
        self.line_msg = lineMessage()
    ...
    def process_frame(self):
        ...
        print(f"Mean value of V channel: {mean_val}")
        if 120 > mean_val > 100:
        ...
        self.frame = self.adjust_gamma(self.frame, gamma=gamma)
        ...
        self.frame = cv2.cvtColor(limg, cv2.COLOR_LAB2BGR)
        ...
        if detected_points.shape[0] > 1:
            ...
            self.line_msg.angle = Float32(angle)
            self.line_msg.pixel = Int16(deviation)
...
if __name__ == '__main__':
    rospy.init_node('zed2_lane_detection', anonymous=True)
    zld = ZED2LaneDetection()
